...
export async function POST(req: NextRequest) {
  try {
    ...
    if (imageFile) {
          ...
          const uploadResult = await new Promise<UploadApiResponse>((resolve, reject) => {
            const stream = cloudinary.uploader.upload_stream(
              {
                folder: "volunteer-jobs",                // Cloudinary folder
    ...
    do {
      ...
      const existing = await prisma.volunteer_Job.findUnique({ where: { slug } });
      if (!existing) isUnique = true;
    } while (!isUnique);

    const volunteerJob = await prisma.volunteer_Job.create({
      data: {
        title,
        content,
        admin_id: adminId,
    ...
    return NextResponse.json(volunteerJob, { status: 201 });
  } catch (error) {
...
export async function GET() {
  try {
    const volunteerJobs = await prisma.volunteer_Job.findMany({
      orderBy: { date: "desc" }, // newest first
    });
    return NextResponse.json(volunteerJobs, { status: 200 });
  } catch (error) {
