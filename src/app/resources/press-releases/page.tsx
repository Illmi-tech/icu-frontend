// src/app/resources/press-releases/page.tsx
'use client';
...
// This should match the PressRelease type returned by your API
type PressRelease = {
  id: number;
  slug: string;
  title: string;
...
export default function PressReleasesPage() {
  ...
  useEffect(() => {
    const fetchPressReleases = async () => {
      try {
        const res = await fetch('/api/press-releases'); // Adjust if your API is deployed separately
    ...
    <section className="py-16 px-4 md:px-8 bg-[#f9fafb] min-h-screen">
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          ...
          <p className="text-center text-gray-500 text-lg">No press releases yet. Check back soon!</p>
        ) : (
          <div className="grid md:grid-cols-2 gap-8">
            {pressReleases.map((pressRelease, i) => (
              ...
              >
                <Link href={`/resources/press-releases/${pressRelease.slug}`}>
                  <Image
                  ...
                  <div className="p-5">
                    ...
                    <p className="text-gray-600 mt-2">{pressRelease.excerpt}</p>
